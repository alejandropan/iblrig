...
def update_fig(f, axes, tph):
    ...
    plot_rew_psych(psych_data, ax=ax_psych)
    plot_rew_chron(chron_data, ax=ax_chron)
...
def get_psych_data(tph): ##All rew lines by @alejandro
    ...
    rew_probability_left_buffer =  np.array(tph.rew_probability_left_buffer)
    ...
    def get_prop_ccw_rew(rew_prob_left):
        ntrials_ccw = np.array([
            sum(response_side_buffer[(
                rew_probability_left_buffer == rew_prob_left) & (
                signed_contrast_buffer == x)] < 0) for x in sig_contrasts_all])
        ntrials = np.array([
            sum((signed_contrast_buffer == x) & (
                rew_probability_left_buffer == rew_prob_left))
    ...
    prop_rew_ccw05 = get_prop_ccw_rew(0.5)  #Addition @alejandro
    prop_rew_ccw07 = get_prop_ccw_rew(0.7) #Addition @alejandro
    prop_rew_ccw1 = get_prop_ccw_rew(1) #Addition @alejandro

    return sig_contrasts_all, prop_resp_ccw02, prop_resp_ccw05, prop_resp_ccw08, prop_rew_ccw07, prop_rew_ccw1, prop_rew_ccw05
...
def get_chron_data(tph):
    ...
    rts02, rts05, rts08, rts_rew07, rts_rew1, rts_rew05 = get_rts(0.2), get_rts(0.5), get_rts(0.8), get_rew_rts(0.7), get_rew_rts(1), get_rew_rts(0.5)

    return sig_contrasts_all, rts02, rts05, rts08, rts_rew07, rts_rew1, rts_rew05
...
def plot_rew_psych(psych_data, ax=None):
    if ax is None:
        # f = plt.figure()  # figsize=(19.2, 10.8), dpi=100)
        ax = plt.subplot2grid((1, 1), (0, 0), rowspan=1, colspan=1)
    ax.cla()
    ...
    ax.plot(x, y04, c='g', label='CCW responses Rew 70/100', marker='o', ls='-')
    ax.plot(x, y08, c='b', label='CCW responses Rew 100/70', marker='o', ls='-')
...
def plot_psych(psych_data, ax=None):
    ...
    ax.plot(x, y05, c='k', label='CCW responses 50/50',
            marker='o', ls='-', alpha=0.5)
    ax.plot(x, y02, c='g', label='CCW responses 40/80', marker='o', ls='-')
    ax.plot(x, y08, c='b', label='CCW responses 80/80', marker='o', ls='-')
...
def plot_rew_chron(chron_data, ax=None):
    if ax is None:
        # f = plt.figure()  # figsize=(19.2, 10.8), dpi=100)
        ax = plt.subplot2grid((1, 1), (0, 0), rowspan=1, colspan=1)
    ax.cla()
